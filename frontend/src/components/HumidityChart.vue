...
const updateChart = (data) => {
  ...
  if (existingIndex !== -1) {
    ...
    newValues = newValues.map((val, idx) => 
      idx === existingIndex ? humidity : val
    )
    console.log('🔄 更新已存在的数据点:', timeLabel, '=', humidity, '%')
  } else {
    // 添加新数据点到末尾
    newLabels.push(timeLabel)
    newValues.push(humidity)
    console.log('➕ 添加新数据点:', timeLabel, '=', humidity, '%')
  }
  ...
  console.log(`✅ 湿度图表更新完成，当前数据点数量:${chartData.value.labels.length}`)
...
const initFromProps = () => {
  ...
  const values = recentData.map(d => d.humidity)

  chartData.value.labels = labels
  chartData.value.datasets[0].data = values
  console.log(`💧 湿度图表初始化完成，显示最新${labels.length}条数据`)
  console.log('💧 湿度图表 labels:', labels)
  console.log('💧 湿度图表 values:', values)
}

// ✅ 处理新数据（立即更新图表）
const handleNewData = (msg) => {
  console.log('💧 湿度图表收到 WebSocket 消息:', msg)
  const data = msg.data || msg
  
  if (data.humidity !== undefined && data.timestamp) {
    updateChart(data)
  } else {
    console.log('⚠️ 湿度数据格式不正确，跳过:', data)
  }
}

// ✅ 监听历史数据变化
watch(() => props.initialData, (newData) => {
  console.log('👁️ 湿度图表监听到历史数据变化，数据条数:', newData?.length)
